...
export default function analytics() {
  const sampleData = dataPool();
  const [chartData, setChartData] = useState(sampleData.slice(sampleData.length -5, sampleData.length -1));
  const [modalVisible, setModalVisible] = useState(false);
  const [customRangePickerModelVisible, setCRMVisible] = useState(false);
  const [selectedOption, setSelectedOption] = useState('Month to date');
  ...
  function handleOptionSelect(option) {
    ...
    switch (option.toLowerCase()) {
      case "Month to date".toLocaleLowerCase(): {
        console.log(option);
        setChartData(sampleData.slice(sampleData.length -5, sampleData.length -1));
        break;
      }
      case "Last month".toLocaleLowerCase(): {
        console.log(option);
        setChartData(sampleData.slice(sampleData.length -9, sampleData.length -5));
        break;
      }
    }
    hide();
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Your screen time pattern:</Text>
      ...
      <Modal visible={modalVisible} animationType="slide" onRequestClose={hide}>
        <SafeAreaView style={modalStyles.modalContainer}>
          ...
          {["Month to date", "Last month"].map((range) => (
